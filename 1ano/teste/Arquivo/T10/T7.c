...
//PARAMETRO: c QUE É O CONJUNTO DE NOTAS
...
//PARAMENTRO: c QUE É O CONJUNTO DE SAQUES DE CADA CLIENTE
//          E q QUE É A QUANTIDADE DE CLIENTES QUE FEZ SAQUE
//-------------------------------------------------------------------------------------------------------------------------------
...
//PARAMETRO: c QUE É O CONJUNTO DE REGISTRO DOS SAQUES
...
void f_saque(dados c[], int *q, notas *n){
    ...
    if (ver>0){
        posi = verifica_conta(cc);                                     //     <-------VERIFICAR ESSA PARTE DEPOIS
                                                                        //     PARA VERIFICAR SE A CONTA ESTÁ CADASTRADA
                                                                        //      ESSA PARTE É DA RAFA! !
        if (posi > -1){
            system("cls");
            printf("\n\n\t-----SISTEMA BANCARIO/SAQUE-----\n");
...
int procura_posicao_novetor_saq (char conta[], dados c[], int q)
{
    ...
    int ver=-1;//vai obter a posicao no vetor onde se encontra
                // se é que o cliente dessa conta fez saque.
    for (i=0;i<q;i++)
    {
