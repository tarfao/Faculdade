...
{
    printf("----------------------------------------------------------------\n");
    printf("\tFabricante: %s\n\tMODELO: %s\n\tPLACA: %s\n\tANO DE FABRICAÇÃO: %d\n\tNOME DO CLIENTE: %s\n",c.fabricante,c.modelo,c.placa_car,c.ano_fabricacao,c.nome);
    printf("----------------------------------------------------------------\n");
}
...
void mostra_todos ()
{
    ...
    if ((p = fopen("vendacarro.dat","rb")) == NULL)
    {
        ...
        do
        {
            if (fread(&s,sizeof(struct VENDA_CARRO),1,p) == 1){
                printf("----------------------------------------------------------------\n");
                printf("\tFabricante: %s\n\tMODELO: %s\n\tPLACA: %s\n\tANO DE FABRICAÇÃO: %d\n\tNOME DO CLIENTE: %s\n",s.fabricante,s.modelo,s.placa_car,s.ano_fabricacao,s.nome);
                printf("----------------------------------------------------------------\n");
            }
        }while(!feof(p));
        fclose(p);
                ...
                if (fread(&s,sizeof(struct VENDA_CARRO),1,p) == 1){
                    if (tipo == 1){//tipo fabricante
                        if (strcmp(s.fabricante,k.fabricante) == 0){
                            //printf("%s   %s--- %s   %s",s.fabricante,s.modelo,k.fabricante,k.modelo);
                            //system("pause");
                            c[i] = s;
                            i++;
                        }
...
void ordena_modelo2(struct VENDA_CARRO *c, int q)
{
    ...
    for(cont=0;cont<q-1;cont++){
        if (strcmp(c[cont].modelo,c[cont+1].modelo)>0)
        {
            printf("tr %s  %s ---- %s   %s",c[cont].fabricante,c[cont].modelo,c[cont+1].fabricante,c[cont+1].modelo);
            system("pause");
            trocar1(c,cont,cont+1);
            printf("%s  %s >>> %s   %s ",c[cont].fabricante,c[cont].modelo,c[cont+1].fabricante,c[cont+1].modelo);
            system("pause");
        }
    }
}
            ...
            if(ftell(p)>0){
                ...
                if (fread(&k,sizeof(struct VENDA_CARRO),1,p) == 1){
                    ...
                    ordena_modelo2(conj,q);
                      for(r=0;r<q;r++)
                    {
                        printf("an>>> %s   %s    %d\n",conj[r].fabricante,conj[r].modelo,i);
                    }
                    system("pause");
                    for (i=0;i<q;i++)
                    {
